...
// A pulsing, glowing mouse-style pointer that is hollow in the middle with purple borders — so viewers
// can see exactly what is being pointed at and underlying button/link text is never covered by a solid dot.
const CALLOUT_ON = (finder) => `(()=>{const el=(${finder})(); if(!el) return false; document.getElementById('kbs-dot')?.remove(); el.classList.add('kbs-outline'); if(!document.getElementById('kbs-style')){const s=document.createElement('style'); s.id='kbs-style'; s.textContent='@keyframes kbs-outline-pulse{0%,100%{box-shadow:0 0 8px rgba(76,61,171,.7)!important}50%{box-shadow:0 0 18px rgba(91,70,214,1)!important}}@keyframes kbs-pointer-pulse{0%,100%{filter:drop-shadow(0 0 4px rgba(76,61,171,.85)) drop-shadow(0 0 10px rgba(76,61,171,.55));transform:translate(-3px,-2px) scale(1)}50%{filter:drop-shadow(0 0 8px rgba(91,70,214,1)) drop-shadow(0 0 18px rgba(91,70,214,.9));transform:translate(-3px,-2px) scale(1.08)}}.kbs-outline{outline:3.5px solid #4c3dab!important;outline-offset:3px!important;box-shadow:0 0 14px rgba(76,61,171,.8)!important;border-radius:6px!important;animation:kbs-outline-pulse 1.4s ease-in-out infinite!important}#kbs-dot{position:fixed;z-index:2147483647;pointer-events:none;transform-origin:3px 2px;animation:kbs-pointer-pulse 1.4s ease-in-out infinite}'; document.head.appendChild(s);} const r=el.getBoundingClientRect(); const cx=r.x+r.width/2, cy=r.y+r.height/2; const d=document.createElement('div'); d.id='kbs-dot'; d.style.left=cx+'px'; d.style.top=cy+'px'; d.innerHTML='<svg width="48" height="48" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 2 L3 18.5 L7.2 14.6 L9.8 20.8 L12.7 19.5 L10.1 13.4 L16 13.4 Z" fill="none" stroke="#ffffff" stroke-width="5" stroke-linejoin="round" stroke-linecap="round"/><path d="M3 2 L3 18.5 L7.2 14.6 L9.8 20.8 L12.7 19.5 L10.1 13.4 L16 13.4 Z" fill="none" stroke="#4c3dab" stroke-width="3" stroke-linejoin="round" stroke-linecap="round"/></svg>'; document.body.appendChild(d); return true;})()`;
