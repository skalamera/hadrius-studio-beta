...
- `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` / `GOOGLE_REFRESH_TOKEN` — optional; when set, every rendered video (both **Render MP4** and **Render + Article**) is also uploaded into the **Hadrius Academy** Shared Drive — the module subfolder matching the workflow's module (Testing Program, People Oversight, Branches, Communications, Marketing, Account Surveillance, or Other) — as an "Anyone with the link" viewer copy, linked from the Recorded tab next to the Pylon article. One-time setup for stephen@hadrius.com's account:
  ...
  2. Authorize scope `https://www.googleapis.com/auth/drive` (full Drive access is required to write into an existing Shared Drive folder by id — `drive.file` alone can't see folders it didn't create) as stephen@hadrius.com and get a refresh token — either via the [OAuth 2.0 Playground](https://developers.google.com/oauthplayground) (gear icon → "Use your own OAuth credentials" → your client ID/secret → authorize that scope → "Exchange authorization code for tokens"), or by hitting the standard `https://accounts.google.com/o/oauth2/v2/auth` authorization endpoint yourself with `access_type=offline&prompt=consent` and exchanging the resulting code at `https://oauth2.googleapis.com/token`.
  3. Put all three values in `.env` as `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REFRESH_TOKEN`, then restart the bridge.
  The Hadrius Academy drive id and its module folder ids are hardcoded in `tools/gdrive.mjs` (`HADRIUS_ACADEMY_DRIVE_ID` / `MODULE_FOLDER_IDS`) — update them there if the drive is ever recreated or folders renamed. `GOOGLE_DRIVE_FOLDER_ID` overrides the per-module routing entirely and uploads everything into one specific folder instead, if that's ever wanted.
